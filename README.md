...
7. 가위,바위,보 - 가위,바위,보 버튼 클릭시 승/패 결과 및 점수를 보여주며 2초동안 정지되었다가 다시 가위,바위,보가
   실행되게 되는데 정지가 되는 경우에도 버튼 클릭이 가능하여 클릭시 점수 변동이 일어나는 문제가 있어, 버튼을 클릭 후
   다음 가위,바위,보로 넘어가기 전까지 버튼을 클릭할 수 없도록 변경
...
9. 리액트, webpack 기본 설정 방법
   ...
   9.4 명령어 : npm i react react-dom 명령어를 터미널에 입력(리액트랑 리액트 돔을 설치하겠다는 의미이다.)
